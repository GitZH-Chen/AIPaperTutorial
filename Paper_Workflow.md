...
- [使用 AI 完成体力式论文写作：Human-in-the-loop 的顶会顶刊工作流](#使用-ai-完成体力式论文写作human-in-the-loop-的顶会顶刊工作流)
  ...
  - [5. 最重要的一节：不要直接使用 AI 的第一稿](#5-最重要的一节不要直接使用-ai-的第一稿)
...
## 5. 最重要的一节：不要直接使用 AI 的第一稿

**这是全文最重要的一节。**

至少截至 GPT-5.6 Solar Ultra，AI 独立写出的论文文本通常质量很差，远未达到可以直接使用的标准。即使你已经告诉它应该怎么写，第一稿仍然很可能空泛、重复、逻辑松散并充满“AI 味”。因此，不要期待 AI 一次完成任何写作任务，也不要把“AI 已经生成了一段文字”误认为“这一段已经写完了”。

AI 的价值是快速生成一个供人批判和修改的初稿。无论是论文正文、proof、review、rebuttal 还是 camera-ready revision，首先都要告诉 AI 具体怎么做并提供充分上下文，然后通过多轮讨论、局部修改和人工重写，把初稿改成准确、清晰、可以投稿的文本。如果最终版本仍与 AI 第一稿基本相同，通常说明作者投入的判断和迭代还不够，甚至可能说明作者本人对所写内容一窍不通。**把这样的草稿给专业的人看，无异于裸奔，因为它会毫无遮掩地暴露作者对内容缺乏基本的理解。**正确的工作流如下：
